...
-----------------------------------------------------------
Date      Reason
2022/9/4  release added shipItems
-----------------------------------------------------------
...
namespace sdds {
   ...
   void AidMan::sort()
   {
      iProduct* temp;
      int j{}, i{};
      for (i = 0; i < m_iProductItems; i++)
      {
         for (j = i + 1; j < m_iProductItems; j++)
         {
            if ((m_array[j]->qtyNeeded() - m_array[j]->qty()) > (m_array[i]->qtyNeeded() - m_array[i]->qty())) {
               temp = m_array[i];
               m_array[i] = m_array[j];
               m_array[j] = temp;
            }
   ...
   {
      int year = 0, month = 0, day = 0, counter = 0, row = 0;
      //ut.testMode(true);
      ut.getSystemDate(&year, &month, &day);
      ofstream file("shippingOrder.txt");
      file << "Shipping Order, Date: " << year << "/";
      file << setw(2) << setfill('0') << month << "/" << setw(2) << setfill('0') << day << endl;
      displayHeader(file);
      for (int i = 0; i < m_iProductItems; i++)
      {
         if (m_array[i]->qty() == m_array[i]->qtyNeeded())
         {
            row++;
            file.setf(ios::right);
            file << setfill(' ') << setw(4) << row << " | ";
   ...
   void AidMan::displayItem(int index, ostream& ostr)
   {

      m_array[index]->linear(true);
      ostr << *m_array[index];
      ostr << endl;
   }

   std::ostream& AidMan::displayHeader(ostream& ostr)
   {
   ...
   std::ostream& AidMan::displayFooter(std::ostream& ostr)
   {
      ostr << "-----+-------+-------------------------------------+------+------+---------+-----------" << endl;
